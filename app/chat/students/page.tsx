...
const roomInfo = {
  students: {
    name: 'Student Support Room',
    description: 'Academic stress, pressure, and student life challenges',
    rules: ['Share study tips', 'No comparing grades', 'School-life balance']
  },
};
...
export default function StudentsChatPage() {
  ...
  useEffect(() => {
    try { setIsAdmin(localStorage.getItem('isAdmin') === 'true'); } catch (e) { setIsAdmin(false); }
  }, []);
      ...
      try {
        ...
        if (data) {
          ...
          const mappedMessages = data.map((msg: any) => ({
            ...msg,
            user_name: msg.users?.name || msg.users?.username || msg.user_name || 'Anonymous'
          }));
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const displayName = await getUserDisplayName(user.id, supabase);
      ...
      const tempMessage: Message = {
        ...
        room_id: roomId,
        user_id: user.id,
        user_name: displayName || user.anonymous_name || user.name || 'Anonymous',
        message: messageText,
  ...
  // ADMIN: Delete message as admin via RPC
  const deleteAsAdmin = async (messageId: string, hardDelete: boolean = false) => {
    try {
      ...
      if (payload && payload.error) {
        alert(`Admin delete failed: ${payload.error}`);
        return;
      }
      ...
      setMessages(prev => prev.map(m => m.id === messageId ? {
        ...m,
        is_deleted: true,
        message: payload?.message || '[Message deleted by admin]',
        deleted_at: payload?.deleted_at || new Date().toISOString()
      } : m));

    } catch (err) {
      console.error('Admin delete error:', err);
      alert(`Admin delete error: ${err instanceof Error ? err.message : String(err)}`);
    }
  };
          ...
          {messages.length === 0 ? (
            ...
            <div className="space-y-4">
              {messages.map((msg, index) => (
                <div key={`${msg.id}-${index}`} className={`flex ${msg.user_id === currentUser.id ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-xs` }>
                    <MessageItem
                      message={msg}
                      currentUserId={currentUser.id}
                      isAdmin={isAdmin}
                      onDeleteAsAdmin={async (id, hard) => await deleteAsAdmin(id, !!hard)}
                    />
                  </div>
                </div>
              ))}
