...
const roomInfo = {
  anxiety: {
    name: 'SpritualSupport Room',
    description: 'Discuss prayerLife, ',
    rules: ['Be kind and supportive', 'No medical advice', 'Respect privacy']
  },
  // ... other rooms
};

export default function SpritualSupport() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Replace temp message with real one and mark as added
      if (data && data[0]) {
        // Add the real ID to our tracking set so real-time doesn't add it again
      ...
      <main className={`flex min-h-screen w-full max-w-5xl flex-col py-8 px-4 ${currentThemeConfig.bg}`}>
        ...
        <div className="flex gap-2">
          <input 
            ...
            className={`flex-1 border rounded-full px-4 py-3 ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-gray-100 border-gray-300 text-slate-900'}`}
          />
          <button 
            onClick={handleSendMessage}
            disabled={!newMessage.trim()}
            className={`${currentThemeConfig.button} text-white px-6 py-3 rounded-full hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed`}
          >
            Send
          </button>
        </div>
        ...
        <div className={`mt-4 p-3 rounded border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-100 border-gray-300'}`}>
